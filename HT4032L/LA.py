...
if not uint32_typecode:
	raise Exception("This platform has no typecode for uint32 arrays")
...
class LA(object):
	...
	def __init__(self, driver="auto", index=0, dump=False):
		...
		self.status = array(uint32_typecode, "\x00"*1024)
	...
	def Poll(self):
		...
		while retry_count<5 and outsize<0x100:
			...
			if not len(data):
				...
				continue
			retry_count = 0
			chunk = array(uint32_typecode, data)
			for val in chunk:
				if got_start:
					if val==DATA_END:
						break
